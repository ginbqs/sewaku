...
$(document).ready(function () {
  ...
  $("#menu_access_management_config").addClass('active');
  table_config = $("#manage_all").DataTable({
    processing: true,
    serverSide: true,
    ajax: '{!! route('admin.allConfig.config') !!}',
    "columnDefs": [
      { 
...
function reload_table() {
    table_config.ajax.reload(null, false); //reload datatable ajax
}
function create(){
  $("#modal_data").empty();
  $('.modal-title').text('Tambah Config');
...
$("#manage_all").on("click", ".edit", function () {
    $("#modal_data").empty();
    $('.modal-title').text('Edit Config'); // Set Title to Bootstrap modal title
    $("#modal-overlay").addClass('overlay d-flex justify-content-center align-items-center');
    $("#modal-overlay-content").addClass('fas fa-2x fa-sync fa-spin');
    var id = $(this).attr('id');
